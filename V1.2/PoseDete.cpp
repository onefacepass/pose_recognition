...
int PoseDete::DetectPose(cv::Mat &frame, int *& _rect, std::vector<cv::Point2f> &skeleton_points) {
	...
	if (zuobiao4_id % 4 == 0)//不够完善 无法摒弃10位长的乱码，
	{
		...
		if (!facescope_temp.empty())
		{
			...
			*(_rect + 3) = facescope_temp[3];
			...
			//get coordinate
			for (size_t i = 0; i < poseRes.size(); i++)
			{
				skeleton_points.insert(skeleton_points.begin() + i, cv::Point2f(poseRes[i].x, poseRes[i].y));
			}
	...
	{
		_rect = nullptr;
		return -3;// have people ,no rise hand
	}
